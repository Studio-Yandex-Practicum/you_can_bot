...
class BaseCaseForQuestionTests(APITestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.task = Task.objects.create(number=cls.TASK_CORRECT,
                                       end_question=cls.END_QUESTION)
        cls.question = Question.objects.create(
            task=cls.task, number=cls.QUESTION_NUMBER,
            content="Просто вопрос", example=""
        )
        cls.question_2 = Question.objects.create(
            task=cls.task, number=2,
            content="вопрос№2", example=""
        )
        choices = (Choice(question=cls.question, title="Заголовок1",
                          description="Описание1"),
                   Choice(question=cls.question, title="Заголовок2",
