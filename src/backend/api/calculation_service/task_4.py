...
def _transform_answers_to_dict(user_answers: list[Answer]) -> dict[int: int]:
    return {
        int(answer.question.number): int(answer.content)
        for answer in user_answers
    }


def _calculate_scales_avg_score(user_answers: dict[int: int]) -> dict[
                                                                 str: float]:
    scales_avg_score = {}
    for scale, questions in SCALES.items():
        scale_total_score = sum(
            user_answers[question] for question in questions)
...
def _get_top_features_sorted(scales_avg_score: dict[str: float]) -> list[str]:
    ...
    filtered_scales = [
        scale for scale in sorted_scales if scale[1] > MIN_AVG_SCORE
    ]
